...
public class WaterDeathAnimator implements DeathAnimator {
    ...
    public void animate(Animal animal, long now) {
        if (animal.getState() == Animal.State.waterDeath) {
            ...
            if (deathTime == 1) {
                animal.setImage(new Image(Animal.WATER_DEATH1, FROG_SIZE, FROG_SIZE, true, true));
            }
            if (deathTime == 2) {
                animal.setImage(new Image(Animal.WATER_DEATH2, FROG_SIZE, FROG_SIZE, true, true));
            }
            if (deathTime == 3) {
                animal.setImage(new Image(Animal.WATER_DEATH3, FROG_SIZE, FROG_SIZE, true, true));
            }
            if (deathTime == 4) {
                animal.setImage(new Image(Animal.WATER_DEATH4, FROG_SIZE, FROG_SIZE, true, true));
            }
            if (deathTime == 5) {
                animal.respawn();
                deathTime = 0;
            ...
            System.out.println("Water death");
